...
export const surveyCalculatorCopy = {
  ...
  ru: {
    heading: {
      eyebrow: 'Предварительный расчёт стоимости',
      title: 'Калькулятор стоимости предпокупочного сюрвейерского осмотра',
      introduction:
        'Укажите данные яхты и выберите объём осмотра. Результат представляет собой ориентировочную стоимость профессиональных услуг, а не обязательное ценовое предложение.',
    },
    fields: {
      loa: 'Габаритная длина яхты, метры',
      loaPlaceholder: 'Например: 12,5',
      loaHelp:
        'Автоматический расчёт доступен для яхт длиной от 5 до 40 метров.',
      individualQuote: 'Запросить индивидуальное предложение',
      vesselType: 'Тип яхты',
      vesselTypeHelp:
        'Для катамаранов, многокорпусных и других яхт с нестандартной конфигурацией результат служит только первоначальным ориентиром. Окончательная стоимость подтверждается после рассмотрения данных яхты.',
      package: 'Объём осмотра',
      hullInspection: 'Осмотр корпуса',
      hullHelp: 'Выберите предполагаемые условия доступа для осмотра.',
      fullHullNote:
        'Подъём яхты и осмотр вне воды входят в полный пакет, поэтому при выборе этого пакета данный вариант фиксирован.',
      additional: 'Дополнительные виды осмотра',
      additionalHelp:
        'Выберите дополнительные виды проверки для индивидуального объёма осмотра.',
      location: 'Местонахождение яхты',
      locationPlaceholder: 'Марина, город и страна',
      locationHelp:
        'Расходы на поездку и затраты, связанные с местонахождением яхты, подтверждаются отдельно в окончательном предложении.',
    },
    labels: {
      vesselType: {
      ...
      packageType: {
        base: 'Базовый сюрвейерский осмотр',
        custom: 'Индивидуальный объём осмотра',
        full: 'Полный пакет осмотра',
      },
      packageDescription: {
        base: 'Базовый предпокупочный осмотр состояния с выбранным осмотром на плаву или вне воды.',
        custom:
          'Базовый осмотр с индивидуально выбранными дополнительными видами проверки.',
        full: 'Базовый осмотр, подъём яхты, ходовые испытания, осмотр двигателей и такелажа, где применимо, с учётом утверждённой скидки за пакет.',
      },
      hullInspection: {
        afloat: 'Осмотр на плаву',
        'hull-out': 'Подъём яхты и осмотр вне воды',
      },
      item: {
        'base-survey': 'Базовый предпокупочный осмотр состояния',
        'afloat-inspection': 'Осмотр на плаву',
        'hull-out-inspection': 'Подъём яхты и осмотр вне воды',
        'sea-trial': 'Ходовые испытания',
        'engine-inspection': 'Осмотр двигателей',
        'rigging-sails-inspection': 'Осмотр такелажа и парусов',
      },
      itemDescription: {
        'sea-trial': 'Оценка работы яхты на ходу, когда это позволяют условия.',
        'engine-inspection':
          'Осмотр доступных механизмов и элементов силовой установки.',
        'rigging-sails-inspection': 'Доступно только для парусных яхт.',
      },
    },
    calculate: 'Рассчитать стоимость',
    result: {
      eyebrow: 'Ориентировочный расчёт',
      heading: 'Ориентировочная стоимость сюрвейерского осмотра',
      priceLabel: 'Ориентировочная стоимость профессионального осмотра',
      loa: 'Габаритная длина яхты',
      vesselType: 'Тип яхты',
      package: 'Выбранный объём осмотра',
      hullInspection: 'Осмотр корпуса',
      location: 'Местонахождение яхты',
      reference: 'Номер расчёта',
      date: 'Дата создания расчёта',
      included: 'Включённые виды осмотра',
      discountHeading: 'Учтена скидка за полный пакет',
      discountBody:
        'Полный пакет включает скидку 15 % по сравнению с отдельным заказом входящих в него видов осмотра.',
      discountLabel: 'Округлённая сумма скидки:',
      exclusionsHeading: 'Важные исключения',
      exclusions: [
        'Услуги верфи и подъём яхты, сборы марины, топливо, эксплуатация яхты, капитан или экипаж для ходовых испытаний, поездки, проживание, анализ масла, специализированная диагностика, ремонт и услуги третьих лиц не включены, если иное не подтверждено письменно.',
        'Применимые налоги будут указаны в окончательном предложении.',
        'Калькулятор предоставляет только предварительный расчёт и не является ценовым предложением, договором или счётом. Окончательная стоимость зависит от характеристик, возраста, конструкции, систем, местонахождения и доступности яхты, условий подъёма, требований к ходовым испытаниям и окончательно согласованного объёма осмотра.',
      ],
      requestQuote: 'Запросить точное предложение',
      whatsapp: 'Отправить запрос в WhatsApp',
      edit: 'Пересчитать',
      reset: 'Новый расчёт',
      privacy:
        'Расчёт хранится только в текущем сеансе браузера и передаётся в форму запроса лишь после перехода к запросу предложения.',
    },
    client: {
      locale: 'ru-RU',
      metres: 'метров',
      validation: {
        'missing-or-invalid-loa':
          'Укажите корректную длину яхты. Минимальная поддерживаемая длина — 5 метров.',
        'below-minimum-loa':
          'Минимальная поддерживаемая длина яхты — 5 метров. Для яхты меньшей длины запросите индивидуальное предложение.',
        'above-maximum-loa':
          'Калькулятор предназначен для яхт длиной до 40 метров. Для яхты большей длины запросите индивидуальное предложение All Yacht Service.',
        'invalid-vessel-type': 'Выберите корректный тип яхты.',
        'invalid-package': 'Выберите корректный объём осмотра.',
        'invalid-hull-inspection':
          'Выберите корректный вариант осмотра корпуса.',
        'invalid-options': 'Выберите корректные дополнительные виды осмотра.',
      } satisfies Record<SurveyValidationCode, string>,
      liveResult: (price: string, reference: string) =>
        `Ориентировочная стоимость сюрвейерского осмотра: ${price}. Номер расчёта: ${reference}.`,
    },
  },
} as const;
...
export const deliveryCalculatorCopy = {
  ...
  ru: {
    heading: {
      eyebrow: 'Предварительный расчёт маршрута и стоимости',
      title: 'Калькулятор стоимости профессионального перегона яхты',
      introduction:
        'Выберите порт отправления и назначения из списка, затем укажите данные яхты и условия перегона. Результат является ориентировочным и не имеет обязательной силы.',
    },
    route: {
      legend: 'Маршрут',
      departure: 'Порт отправления',
      departurePlaceholder: 'Поиск порта, например Altea, Barcelona или Cannes',
      departureAria: 'Порты отправления',
      departureHelp: 'Начните ввод и выберите порт отправления из списка.',
      destination: 'Порт назначения',
      destinationPlaceholder: 'Поиск порта, например Palma, Genoa или Athens',
      destinationAria: 'Порты назначения',
      destinationHelp: 'Начните ввод и выберите порт назначения из списка.',
      swapAria: 'Поменять местами порт отправления и порт назначения',
      swap: 'Поменять порты',
      coverageBefore:
        'Калькулятор поддерживает указанные порты Средиземноморья и близлежащей части Атлантики. Если нужного порта нет в списке,',
      coverageLink: 'запросите индивидуальный расчёт маршрута',
    },
    fields: {
      yachtType: 'Тип яхты',
      lengthBand: 'Диапазон длины яхты',
      lengthHelp:
        'Окончательное предложение будет основано на точных размерах, водоизмещении, системах и состоянии яхты.',
      complexity: 'Сложность перегона',
      complexityHelp:
        'Выбранная категория даёт только первоначальный ориентир. Окончательная классификация перегона подтверждается после рассмотрения маршрута и яхты.',
      timing: 'Срочность',
      timingHelp:
        'Срочный перегон предполагает ускоренную подготовку или мобилизацию и зависит от доступности команды.',
    },
    labels: {
      yachtType: {
        sailing: 'Парусная яхта',
      ...
      complexity: {
        ...
        complex: 'Сложный перегон или дополнительные формальности',
      },
      complexityDescription: {
        standard:
          'Обычно планируемый прибрежный или средиземноморский перегон без нестандартных формальностей, выявленных при первоначальном запросе.',
        offshore:
          'Более продолжительный или морской переход, требующий дополнительного планирования и учёта эксплуатационных факторов.',
        complex:
          'Перегон с дополнительными границами, формальностями, ограничениями маршрута или другими эксплуатационными сложностями.',
      },
      urgency: {
        standard: 'Стандартный срок',
        urgent: 'Приоритетный перегон',
      },
    },
    calculate: 'Рассчитать перегон',
    result: {
      eyebrow: 'Ориентировочный расчёт',
      ...
      lengthBand: 'Диапазон длины яхты',
      complexity: 'Сложность перегона',
      timing: 'Выбранная срочность',
      reference: 'Номер расчёта',
      date: 'Дата расчёта',
      assumptionsHeading: 'Важные допущения по маршруту',
      assumptions: [
        'Указанное расстояние рассчитано по ориентировочному морскому маршруту с помощью упрощённого графа. Оно не является навигационным маршрутом, планом перехода или заменой актуальных морских карт.',
        'Окончательное планирование перехода должно выполняться с использованием актуальных морских карт, Извещений мореплавателям, метеорологической информации, портовых и пограничных ограничений, а также с учётом фактического состояния, оборудования, автономности, ходовых характеристик яхты и требований к экипажу.',
      ],
      exclusionsHeading: 'Не включённые расходы',
      exclusions: [
        'Ориентировочная начальная стоимость профессионального перегона не включает автоматически топливо, смазочные материалы, провизию, сборы марины или за стоянку, авиабилеты, поездки экипажа, местный транспорт, проживание, таможенные или агентские сборы, оплату каналов или шлюзов, разрешения, ремонт, запасные части, осмотры, подъём яхты, ожидание погоды, ожидание по просьбе владельца и услуги третьих лиц, если иное не подтверждено письменно.',
        'Применимые налоги будут указаны в окончательном предложении.',
      ],
      disclaimerHeading: 'Расчёт не имеет обязательной силы',
      disclaimer:
        'Калькулятор предоставляет только предварительный расчёт и не является навигационным маршрутом, планом перехода, ценовым предложением, договором или счётом. Окончательная стоимость зависит от точных характеристик, состояния и местонахождения яхты, маршрута, требуемого экипажа, подготовки, эксплуатационной сложности, сроков и согласованного объёма перегона.',
      requestQuote: 'Запросить предложение на перегон',
      whatsapp: 'Отправить запрос в WhatsApp',
      edit: 'Пересчитать',
      reset: 'Новый маршрут',
      privacy:
        'Расчёт маршрута хранится только в текущем сеансе браузера и передаётся в форму запроса лишь после перехода к запросу предложения.',
    },
    client: {
      locale: 'ru-RU',
      nauticalMiles: 'морских миль',
      startTyping: 'Начните ввод для поиска порта',
      noPorts: 'Порты не найдены',
      invalidDeparture: 'Выберите корректный порт отправления из списка.',
      invalidDestination: 'Выберите корректный порт назначения из списка.',
      samePorts: 'Порт отправления и порт назначения должны отличаться.',
      routeUnavailable:
        'Не удалось рассчитать ориентировочный морской маршрут между выбранными портами. Запросите индивидуальный расчёт маршрута.',
      whatsapp: {
        heading: 'Расчёт стоимости перегона яхты',
        departure: 'Порт отправления',
        destination: 'Порт назначения',
        distance: 'Ориентировочное расстояние',
        fee: 'Ориентировочная начальная стоимость',
      },
      liveResult: (distance: string, price: string, reference: string) =>
        `Ориентировочное расстояние по морскому маршруту: ${distance}. Ориентировочная начальная стоимость профессионального перегона: ${price}. Номер расчёта: ${reference}.`,
    },
  },
} as const;
